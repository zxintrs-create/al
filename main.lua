...
RunService:BindToRenderStep(
	...
	function()
		...
		local isJumping =
			(Humanoid.FloorMaterial == Enum.Material.Air and vel.Y > 1)
			or (st == Enum.HumanoidStateType.Jumping)
			or (st == Enum.HumanoidStateType.Freefall and vel.Y > 2)
...
local function executePlayback()
	...
	-- Pastikan timestamp replay selalu tersedia.
	...
	-- Digunakan supaya posisi record benar-benar menjadi
	-- referensi utama replay dan karakter tidak jatuh keluar route.
	local replayCFrame = RootPart.CFrame
	...
	RunService:BindToRenderStep(
		...
		function(dt)
			...
			if state.isPaused then
				...
				-- Pertahankan posisi terakhir replay.
				if replayCFrame then
					RootPart.CFrame = replayCFrame
				end
			...
			if playbackState == "WALKING_TO_START"
				...
				local horizontalStart = Vector3.new(
					startPos.X,
					0,
					startPos.Z
				)
				...
				local horizontalDistance =
					(horizontalCurrent - horizontalStart).Magnitude

				-- Jika sudah dekat dengan start,
				-- langsung masuk replay.
				if horizontalDistance <= 1.5 then
					...
					lastTarget = state.timeline[1].Position
				...
				-- Timeout hanya mencegah infinite loop
				-- ketika karakter benar-benar tidak bergerak.
				if tick() > timeoutTimer then
			...
			if playbackState == "PLAYING" then
				...
				-- ---------------------------------------------
				-- Cari node yang sesuai timestamp.
				-- ---------------------------------------------
				...
				if currentIndex >= #state.timeline then
					...
					-- Posisi akhir benar-benar mengikuti record.
					replayCFrame = CFrame.new(
						finalNode.Position
					)
				...
				-- ---------------------------------------------
				-- Jalankan Humanoid Move supaya animasi
				-- movement tetap aktif.
				-- ---------------------------------------------
				...
				-- ---------------------------------------------
				-- DETEKSI JUMP BERDASARKAN RECORD
				-- ---------------------------------------------

				if currentNode.Jump
					and jumpIndex ~= currentIndex then

					if tick() - lastJumpTime > 0.12 then
				...
				-- Node berikutnya juga diperiksa supaya event
				-- jump tidak terlambat ketika timestamp sangat
				-- dekat dengan perpindahan node.
				if nextNode.Jump
					and currentTime >= nextRelative - 0.035
					and jumpIndex ~= currentIndex + 1 then
				...
				--
				-- Ini bagian penting:
				-- posisi Y diambil dari record, bukan dari
				-- physics saat playback.
				--
				-- Dengan begitu karakter tidak jatuh ketika
				-- replay melewati bagian yang direkam di atas
				-- platform/celah.
				-- ---------------------------------------------

				local targetRotation = replayCFrame
				...
				replayCFrame = targetRotation

				-- Posisi mengikuti route record.
				--
				-- Menggunakan CFrame di sini bukan untuk
				-- menghapus Humanoid movement, tetapi untuk
				-- mencegah physics membuat karakter jatuh
				-- dari posisi replay.
				RootPart.CFrame = replayCFrame
